...
if __name__ == '__main__':
    ...
    with open(filename_1, 'r') as f:            
        ...
        for line in f:
            ...
            if cluster_id == u_id:
                print i
                random_reward_list.append(data[0])
                user_reward_list1.append(all_reward_list1[-1] - all_reward_list1[-2])
                tim[i] = i
                if tim[i] !=0:
                    user_rewardRatio_list1.append(data[2]/tim[i])
                else:
                    user_rewardRatio_list1.append(-1)

    print len(user_cluster_dic)
    print sorted(user_cluster_dic.values())
    large_list = []
    for u in user_cluster_dic:
        if user_cluster_dic[u] >= 503:
            large_list.append(u)
    print large_list
    print len(user_cluster_dic.keys())
    ...
    with open(filename_2, 'r') as f_2:            
        ...
        for line in f_2:
            ...
            if cluster_id == u_id:
                #print i
                user_reward_list2.append(all_reward_list2[-1] - all_reward_list2[-2])
                if data[0] !=0:
                    user_rewardRatio_list2.append(data[2]/tim[i])
                else:
                    user_rewardRatio_list2.append(-1)

    user_reward_list1_acc = []
    user_reward_list2_acc = []
    for j in range(len(user_reward_list1)):
        user_reward_list1_acc.append(sum(user_reward_list1[:j]))
        user_reward_list2_acc.append(sum(user_reward_list2[:j]))
    print len(user_reward_list1_acc)
